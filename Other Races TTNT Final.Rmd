...
library(dplyr) # Function to count White and Other Races patients 
count_race <- function(df) { df %>% filter(combined_race_ethnicity %in% c("White", "Other Races")) %>% count(combined_race_ethnicity) %>% mutate(dataset = deparse(substitute(df))) } 
...
saveRDS(b, "Other Races_TTNT table.rds")
```
...
saveRDS(my_plot, "Other Races_TTNT Forest.rds")

```
# Complete data analysis comparison
```{r}
completed_data <- read.csv("Other Races_complete_data_results_ttnt.csv")
...
saveRDS(p, "Other Races_comparison_ttnt")
```
