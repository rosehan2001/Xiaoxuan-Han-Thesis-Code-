...
library(dplyr) # Function to count White and African American patients 
count_race <- function(df) { df %>% filter(combined_race_ethnicity %in% c("White", "African American")) %>% count(combined_race_ethnicity) %>% mutate(dataset = deparse(substitute(df))) } 
...
saveRDS(b, "African American_TTNT table.rds")
```
...
saveRDS(my_plot, "African American_TTNT Forest.rds")

```
# Complete data analysis comparison
```{r}
completed_data <- read.csv("African American_complete_data_results_ttnt.csv")
