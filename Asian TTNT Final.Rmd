...
library(dplyr) # Function to count White and Asian patients 
count_race <- function(df) { df %>% filter(combined_race_ethnicity %in% c("White", "Asian")) %>% count(combined_race_ethnicity) %>% mutate(dataset = deparse(substitute(df))) } 
...
# final_ate_results <- ate_results[[22]] %>% mutate(HR = exp(LogHR),
#                                                   SE = SE, 
#                                                   Lower_CI =exp(LogHR - 1.96 * SE),
#                                                   Upper_CI = exp(LogHR + 1.96 * SE),
#                                                   Variable = "as.factor(treatment)1") %>% select(-LogHR)
```
...
saveRDS(b, "Asian_TTNT table.rds")
```
...
saveRDS(my_plot, "Asian_TTNT Forest.rds")

```
# Complete data analysis comparison
```{r}
completed_data <- read.csv("Asian_complete_data_results_ttnt.csv")
...
saveRDS(p, "Asian_comparison_ttnt")
```
