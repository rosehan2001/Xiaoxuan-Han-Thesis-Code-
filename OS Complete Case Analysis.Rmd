...
os_data$combined_ecog <- as.factor(os_data$combined_ecog)
os_data$combined_ecog <- relevel(os_data$combined_ecog , ref = "0-1" )
os_data$sesindex2015_2019 <- as.factor(os_data$sesindex2015_2019)
os_data$hemoglobin <- as.factor(os_data$hemoglobin)
os_data$marker_calcium_result <- as.factor(os_data$marker_calcium_result)
os_data$marker_creatinie_result <- as.factor(os_data$marker_creatinie_result)
# os_data$lightchainkappa <- as.factor(os_data$lightchainkappa)
# os_data$lightchainlambda <- as.factor(os_data$lightchainlambda)
os_data$practicetype <- as.factor(os_data$practicetype)
# os_data$mproteinigg <- as.factor(os_data$mproteinigg)
...
aa_data <- os_data %>% filter(combined_race_ethnicity %in% c("White", "African American" ))
...
# results 
crude <-coxph(Surv(OS, event) ~ as.factor(treatment), data = aa_data)
a  <- summary(crude)
...
```{r}
# results 
covariate_adj <- coxph(Surv(OS, event) ~ as.factor(treatment) + age_group+
...
nn_match <-coxph(Surv(OS, event) ~ as.factor(treatment), data = match.data_nn)
a <- summary(nn_match)
...
library(survival)
#  results
nn_caliper_match <-coxph(Surv(OS, event) ~ as.factor(treatment), data = match.data_caliper)
...
library(survival)
# ps matching (optimal) results
nn_caliper_match <-coxph(Surv(OS, event) ~ as.factor(treatment), data = match.data_optimal)
a <- summary(nn_caliper_match)
...
cox_model_trimmed_2<- coxph(Surv(OS, event) ~ as.factor(treatment), data = aa_data_trimmed, weights = weightss)
...
cox_model<- coxph(Surv(OS, event) ~ as.factor(treatment), data = aa_data, weights = ps_weights_3)
a <- summary(cox_model)
...
```{r}
doubly_robust <- coxph(Surv(OS, event) ~ as.factor(treatment) + age_group+
               gender + issstage + asct + combined_ecog+
                 risk_group + sesindex2015_2019 +
                 year_diag_group + hemoglobin + marker_calcium_result + marker_creatinie_result
...
dataPlot <- data.frame(variable   = rownames(ExtractSmd(table_unmatched)),
                       ...
                       Matched_optimal  = as.numeric(ExtractSmd(table_optimal)),
                       Matched_caliper_0.1  = as.numeric(ExtractSmd(table_caliper)),
                        Matched_nearest = as.numeric(ExtractSmd(table_nn)),
                       iptw_trimming = as.numeric(ExtractSmd(table_sw_1)),
...
a <- a %>% select(Method,Hazard_Ratio, Lower_CI, Upper_CI);
rownames(a) <- NULL
print(a)
...
write.csv(a, "African American_complete_data_results.csv", row.names = FALSE)
```
#### Forest Plot
```{r}
...
```{r}
# match each race/ethnicity group separately
aa_data <- os_data %>% filter(combined_race_ethnicity %in% c("White", "Asian" ))
aa_data <- aa_data %>% mutate(treatment = ifelse(combined_race_ethnicity == "White",0,1))
...
table(aa_data_trimmed$treatment)
dim(aa_data_trimmed)
# check sample szie after trimming
table(aa_data_trimmed$censored) # In White, 3653 events, 2047 censored 
# check weights summary 
summary(aa_data$weightss)[aa_data$treatment =="0"]
summary(aa_data_trimmed$weightss)[aa_data$treatment =="1"]

boxplot(aa_data$weightss ~ as.factor(aa_data$treatment), main = "Before Trimming", col = c("lightblue", "lightgreen"))
boxplot(aa_data_trimmed$weightss ~ as.factor(aa_data_trimmed$treatment),
        main = "After Trimming", col = c("lightblue", "lightgreen"))

ggplot(aa_data,
       aes(x = weightss, fill = as.factor(treatment))) +
  geom_histogram(alpha = 0.5, bins = 50, position = "identity")
ggplot(aa_data_trimmed,
       aes(x = weightss, fill = as.factor(treatment))) +
  geom_histogram(alpha = 0.5, bins = 50, position = "identity") + scale_x_log10()
...
write.csv(a, "Asian_complete_data_results.csv", row.names = FALSE)
```
#### Forest Plot
```{r}
# Define the desired order manually
...
```{r}
# match each race/ethnicity group separately
aa_data <- os_data %>% filter(combined_race_ethnicity %in% c("White", "Hispanic or Latino" ))
aa_data <- aa_data %>% mutate(treatment = ifelse(combined_race_ethnicity == "White",0,1))
...
write.csv(a, "Hispanic or Latino_complete_data_results.csv", row.names = FALSE)
```
#### Forest Plot
```{r}
# Define the desired order manually
...
```{r}
# match each race/ethnicity group separately
aa_data <- os_data %>% filter(combined_race_ethnicity %in% c("White", "Other Races" ))
aa_data <- aa_data %>% mutate(treatment = ifelse(combined_race_ethnicity == "White",0,1))
...
write.csv(a, "Other Races_complete_data_results.csv", row.names = FALSE)
```
#### Forest Plot
```{r}
# Define the desired order manually
