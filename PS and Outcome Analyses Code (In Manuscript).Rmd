...
```{r}
library(survival)
vars <- c( "gender", "age_group", "combined_ecog", 
           "issstage", "asct", "risk_group",
          "sesindex2015_2019","year_diag_group", "hemoglobin",
          "marker_calcium_result",
          "marker_creatinie_result","practicetype",
         "Deletion.13q", "Ploidy")
os_data <- read.csv("os_data.csv")
os_data <-  os_data %>% mutate(combined_race_ethnicity 
                               = factor(combined_race_ethnicity,
                                            levels = c("White", 
                                                       "Hispanic or Latino",
                                                       "Asian",
                                                       "African American",
                                                       "Other Races",
                                                       "Unknown/Missing")))
os_data <- os_data %>% select("combined_race_ethnicity", 
                              "gender", "age_group", "combined_ecog", 
                              "issstage", "asct", "risk_group",
          "sesindex2015_2019","year_diag_group", "hemoglobin",
          "marker_calcium_result",
          "marker_creatinie_result","practicetype",
...
library(mice)
#Sset "Unknown/missing" as NA
os_data_2 <- os_data 
os_data_2$combined_race_ethnicity[os_data_2$combined_race_ethnicity 
                                  == "Unknown/Missing"] <- NA
...
os_data_2 <- os_data_2 %>% select("combined_race_ethnicity", 
                                  "gender", "age_group", 
                                  "combined_ecog", "issstage", 
                                  "asct", "risk_group",
                                  "sesindex2015_2019",
                                  "year_diag_group", "hemoglobin",
                                  "marker_calcium_result",
                                  "marker_creatinie_result",
                                  "practicetype", "Deletion.13q", 
                                  "Ploidy","OS","event")
...
sum(filtered_imputed_list[[22]]$combined_race_ethnicity == "African American")
...
imputed_values <- sapply(imputed_list, 
                         function(df) df$combined_race_ethnicity[missing_idx])
...
for (i in 1:22) { 
    table1 <- CreateTableOne(vars = vars, strata = "treatment", 
                             data =  filtered_imputed_list[[i]], test = FALSE, smd = TRUE)
...
# visualization
ggplot(proportions_by_variable , aes(x=reorder(Variable,
                                               proportion_exceeding_0.1),
                                               y =proportion_exceeding_0.1)) +
  geom_bar(stat = "identity", fill = "steelblue") + labs(
    title = "Proportions of SMDs Exceeding 0.1 by Covariate",
    x = "Covariate",
    y = "Proportions Exceeding 0.1") +
  theme_minimal() + coord_flip()


# average SMDs across imputations for the unmatched data 
...
for (i in 1:num_imputations) { 
  cox_model <- coxph(Surv(OS, event) ~ as.factor(treatment),
                     data = filtered_imputed_list[[i]]) 
...
# Print results 
crude_results <- data.frame( Variable = names(mean_coefs), 
                             HR = hr_estimates, 
                             Lower_CI = hr_lower_CI, Upper_CI = 
                               hr_upper_CI, se =pooled_se , loghr =mean_coefs)
print(crude_results)
...
for (i in 1:num_imputations) { 
  cox_model <- coxph(Surv(OS, event) ~ as.factor(treatment) + age_group+
               gender + issstage + asct + combined_ecog+
                 ...
                 year_diag_group + hemoglobin + 
                 marker_calcium_result + marker_creatinie_result
...
hr_upper_CI <- exp(upper_CI) 
# Print results 
cov_adj_results <- data.frame( Variable = names(mean_coefs),
                               HR = hr_estimates, 
                               Lower_CI = hr_lower_CI, 
                               Upper_CI = hr_upper_CI , 
                               se =pooled_se , loghr =mean_coefs )
...
# Nerest Neighbor Matching - With Caliper 
...
for (i in 1:22) { 
  # Step 1: Fit a logistic regression PS model to get logit(PS)
...
# Calculate logit(PS) and dynamic caliper
logit_ps <- predict(ps_model, type = "link")
caliper_val <- 0.2 * sd(logit_ps, na.rm = TRUE)

# Step 2: Perform nearest neighbor matching using calculated caliper
...
# Group-level summary for reporting
...
if (i == 1) {
  ...
  table1 <- CreateTableOne(vars = vars, strata = "treatment", 
                           data = match.data, test = FALSE, smd = TRUE)
...
# visualization
ggplot(proportions_by_variable , 
       aes(x=reorder(Variable,proportion_exceeding_0.1),
                                               y =proportion_exceeding_0.1)) +
  geom_bar(stat = "identity", fill = "steelblue") + 
  labs(title = "Proportions of SMDs Exceeding 0.1 by Covariate",
       x = "Covariate",
       y = "Proportions Exceeding 0.1") +
  theme_minimal() + coord_flip()
...
smd_avg_nn0.1_1$Variable <- factor(smd_avg_nn0.1_1$Variable, 
...
a <- ggplot(all_smds_3, aes(x = X1.vs.2, y = Variable)) + 
  ...
  geom_point( aes(x = Mean_SMD, y = Variable), color ="blue", size = 3) +  
  labs(x="SMD",y = "Coavariate") + theme_minimal() + 
  geom_text(aes(x = max(all_smds_3$X1.vs.2+0.05), 
                y = reorder(Variable,proportion_exceeding_0.1),  
                label =  round(proportion_exceeding_0.1,2)))

ggsave("African American_NNM02_SMD_Prop.png", a, width = 8, height = 6)
...
ggplot(all_smds_3, aes(x = X1.vs.2, y = Variable)) + 
  ...
  geom_vline(xintercept = c(0.1), linetype = "dashed", color="red") +
  geom_point( aes(x = Mean_SMD, y = Variable), color ="blue", size = 3) +  
  labs(title = "SMDs Across Imputations",x="SMD",y = "Coavariate") +theme_minimal() + geom_text(aes(x = max(all_smds_3$X1.vs.2+0.05), 
                                  y = reorder(Variable,
                                              proportion_exceeding_0.1),  
                                  label =  round( proportion_exceeding_0.1,2)))

library(ggplot2)
all_eps$treatment <- factor(all_eps$treatment, 
                            levels = c(0, 1), 
...
saveRDS(my_plot, "African American_density.rds")

my_plot <- ggplot(all_eps, aes(x = as.factor(treatment), 
                               y = ePS, fill = as.factor(treatment))) +
  geom_boxplot() +
  labs(x = "Race/Ethnicity", y = "Estimated Propensity Score", 
       fill = "Race/Ethnicity") +
  theme_minimal()

saveRDS(my_plot, "African American_boxplot.rds")
...
hr_upper_CI <- exp(upper_CI) 
# Print results 
nn_0.1_results <- data.frame( Variable = names(mean_coefs),
                              HR = hr_estimates, Lower_CI = hr_lower_CI, 
                              Upper_CI = hr_upper_CI, se =pooled_se , 
                              loghr =mean_coefs )
...
ps_all[[i]] <- ps_df
  ...
  table1 <- CreateTableOne(vars = vars, strata = 
                             "treatment", data = match.data, 
                           test = FALSE, smd = TRUE)
...
ps_combined$treatment <- factor(ps_combined$treatment, levels = c(0, 1), 
                                labels = c("White", "African American"))


plot_nnm0.1 <-  ggplot(ps_combined, aes(x = ps, fill =as.factor(treatment))) +
geom_density(alpha = 0.6) + scale_fill_manual(values = c("White" = "blue", "African American" = "pink")) + 
labs(x = "PS",
     fill ="Race/Ethnicity") +
theme_minimal() + theme(legend.position = "none")
print(plot_nnm0.1)
saveRDS(plot_nnm0.1, "African American_NNM_0.1_PS.rds")
```

# Nerest Neighbor Matching - Without Caliper
```{r}
# create a vector of covariates 
vars <- c( "gender", "age_group", "combined_ecog", "issstage", 
           "asct", "risk_group",
          "sesindex2015_2019","year_diag_group", 
          "hemoglobin","marker_calcium_result",
          "marker_creatinie_result","practicetype",
         "Deletion.13q", "Ploidy")
...
tabMatched_nn$Variable <- factor(tabMatched_nn$Variable, levels = desired_order)
tabMatched_nn <- tabMatched_nn %>% arrange(Variable)
...
ggsave("African American_NNM_SMD_Prop.png", a, width = 8, height = 6)
...
ps_combined <- bind_rows(ps_all)
ps_combined$treatment <- factor(ps_combined$treatment, levels = c(0, 1), labels = c("White", "African American"))
...
saveRDS(plot_nnm, "African American_NNM_PS.rds")
...
ps_density_data$treatment <- factor(ps_density_data$treatment, levels = c(0, 1), labels = c("White", "African American"))


p <- ggplot(ps_density_data, aes(x = ps, fill = as.factor(treatment))) + # Replace with your group var
geom_density(alpha = 0.6)+ scale_fill_manual(values = c("White" = "blue", "African American" = "pink")) +labs(x = "PS",
y = "Density", fill = "Race/Ethnicity") + theme_minimal() 


saveRDS(p, "African American_unmatched_ps.rds")
``` 
...
ggsave("African American_optimal_SMD_Prop.png", a, width = 8, height = 6)
...
geom_density(alpha = 0.6) + scale_fill_manual(values = c("White" = "blue", "African American" = "pink")) +
  ...
  theme_minimal() + theme(legend.position = "none")
print(plot_optimal)
saveRDS(plot_optimal, "African American_optimal_PS.rds")
library(dplyr)
...
ggsave("African American_SIPTW_SMD_Prop.png", a, width = 8, height = 6)
...
ggsave("African American_IPTWTRIM_SMD_Prop.png", a, width = 8, height = 6)
...
avg_weights_df$treatment <- factor(avg_weights_df $treatment, levels = c(0, 1), labels = c("White", "African American"))
...
plot_trimming_iptw <- ggplot(avg_weights_df, aes(x = avg_weight,fill = as.factor(treatment))) +
  geom_density(alpha = 0.6) + scale_fill_manual(values = c("White" = "blue", "African American" = "pink")) + 
  labs(
       x = "IPTW Weight", y = "Frequency",fill ="Race/Ethnicity") + theme_minimal() + theme(legend.position = "none")

saveRDS(plot_trimming_iptw, "African American_trimming.rds")
...
saveRDS(plot_sta_iptw, "African American_sta_iptw.rds")
...
dplyr::mutate(imputation = i)
  ...
  for (s in unique(imputed_subset$strata)) { 
    ...
    imputed_subset$treatment <- factor( imputed_subset$treatment, levels = c(0, 1), labels = c("White", "African American"))
     ...
     saveRDS(my_plot, "African American_strata.rds")
...
labels = c("White", "African American")) + theme_minimal()

saveRDS(plot, "African American_PS stratification.rds")
...
ggsave("African American_STRATA_SMD_Prop.png", a, width = 8, height = 6)
...
saveRDS(b, "African American_table.rds")
```
...
my_plot <- ggplot(c,  aes(y = Method, x = Hazard_Ratio)) + geom_point(size = 2, color = "blue") + # Plot HR 
  ...
  labs(title = "Hazard Ratios for White vs. African American ", x = "Hazard Ratio", y = "Method") + coord_cartesian(xlim = c(0.6,1.1)) + theme_minimal()

saveRDS(my_plot, "African American_HR.rds")
```
...
df <- readRDS("African American_table.rds") %>%
mutate(
...
saveRDS(my_plot, "African American_Forest.rds")
...
saveRDS(my_plot, file = "African American_balance.rds")
```
...
mutate(`Race/Ethnicity` = ifelse(treatment == 0, "White", "African American")) %>%
...
mutate(
Race = ifelse(`Race/Ethnicity` == "White", "White", "African American"),
...
relocate(Method, White,`African American`)
...
`African American` = round(nnm_tbl$Original_N[nnm_tbl$treatment == 1],0)
) 

original_row$White <- as.character(original_row$White)
original_row$`African American`<- as.character(original_row$`African American`)
...
saveRDS(a, "African American_sample loss.rds")
```


# Complete data analysis comparison
```{r}
completed_data <- read.csv("African American_complete_data_results.csv")
...
ggdraw() + draw_image("African American_NNM02_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)
...
ggdraw() + draw_image("African American_NNM_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)
...
ggdraw() + draw_image("African American_optimal_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)
...
ggdraw() + draw_image("African American_SIPTW_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)
...
ggdraw() + draw_image("African American_IPTWTRIM_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)
...
ggdraw() + draw_image("African American_STRATA_SMD_Prop.png"),
ncol = 1,
rel_heights = c(0.1, 1)
)

# Arrange in a 2x3 grid
combined <- plot_grid(p1, p2, p3, p4, p5, p6, ncol = 2)

# Save combined panel
ggsave("African American_combined_panel.png", combined, width = 10, height = 10)
